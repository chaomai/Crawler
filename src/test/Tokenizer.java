...
public class Tokenizer {
	...
	public static String tokenize(String source) {
		...
		ArrayList<String> allTheCapitalWords = new ArrayList<String>();

		// tokenize according to the capitals
		nextToken: for (Iterator<String> allTokens = tokens.iterator(); allTokens
				.hasNext();) {
		...
		try {
			for (int i = 0; i < words.size(); i++) {
				stemmer.setCurrent((String) words.get(i));
				stemmer.stem();
